...
Core tests take longer than any other Netcash tests, due to the high amount of computational work involved in validating core components.
...
To run only Netcash's core tests (after building):
...
To run only Netcash's crypto tests (after building):
...
To run only Netcash's hash tests (after building):
...
To run only Netcash's performance tests (after building):
...
To run only Netcash's unit tests (after building):
